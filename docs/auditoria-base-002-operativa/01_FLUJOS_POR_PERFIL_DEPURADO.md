...
**Subtotal Administrador:**
- COMPLETO: 0 | PARCIAL: 5 | NO VERIFICABLE: 5 | NO IMPLEMENTADO: 1 | SIN CIERRE: 1
- % COMPLETO: 0%
...
**Subtotal Contador:**
- COMPLETO: 0 | PARCIAL: 6 | NO VERIFICABLE: 3 | NO IMPLEMENTADO: 1 | SIN CIERRE: 4
- % COMPLETO: 0%
...
**Subtotal Cobranza:**
- COMPLETO: 0 | PARCIAL: 3 | NO VERIFICABLE: 7 | NO IMPLEMENTADO: 3 | SIN CIERRE: 1
- % COMPLETO: 0%
...
|-------|----------|-------------|
| E0 | 9 | Solo nombre (6 NO IMPLEMENTADO + documentación general) |
| E1 | 18 | Componente/tabla/ruta existe, sin inspección de lógica |
| E2 | 15 | Código Edge Function inspeccionado aisladamente, sin flujo completo |
...
### 1. Rol `admin` en ADM-001 vs. auditoría técnica

**Afirmación anterior:** ADM-001 clasifica como COMPLETO: "admin (role asignado)"

**Hallazgo técnico:** Auditoría técnica encontró que `admin` no existe en enum `member_role` en `init.sql:9`

**Resolución:**
- `create-company/index.ts:59-69` asigna `role = 'admin'` en `company_members`
- Pero `member_role` enum = `['owner', 'accountant', 'supervisor', 'spender', 'collector']` (sin 'admin')
- ✅ **FIX REQUERIDO:** Agregar `'admin'` a enum antes de que ADM-001 pueda ser COMPLETO

**Reclasificación:** ADM-001 → PARCIAL (E2), bloqueado por enum faltante
