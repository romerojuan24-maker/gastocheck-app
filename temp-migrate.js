...
const url = new URL('https://omhycwfjxynkfwywzwvz.supabase.co/rest/v1/rpc/exec_sql');
...
const sql = \$([System.Text.RegularExpressions.Regex]::Escape(-- ============================================================================
-- GastoCheck v1.0 PERFILAMIENTO MÃNIMO
-- Fecha: 2026-06-27
-- DescripciÃ³n: Cambios mÃ­nimos para preparar GastoCheck v1.0 a venta
-- ============================================================================

-- 1. AGREGAR RASTREO DE QUIÃ‰N CAPTURÃ“ CADA GASTO (Multi-Comprador)
-- ============================================================================
...
-- 2. CREAR TABLA VIÃTICOS (Nueva, no existÃ­a)
-- ============================================================================
CREATE TABLE IF NOT EXISTS viaticos (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id UUID NOT NULL REFERENCES companies(id) ON DELETE CASCADE,
  created_by UUID NOT NULL REFERENCES profiles(id) ON DELETE SET NULL,
  person_id UUID REFERENCES profiles(id) ON DELETE SET NULL, -- QuiÃ©n solicita el viÃ¡tico (puede ser diferente a created_by)
  ...
  approved_by UUID REFERENCES profiles(id) ON DELETE SET NULL, -- QuiÃ©n aprobÃ³ (contador_general/supervisor)
...
-- Vista: ViÃ¡ticos por persona (individual)
...
-- Vista: Resumen ejecutivo por empresa y perÃ­odo
...
SELECT
  ...
  -- Dinero en resguardo (pÃ³lizas pendientes)
...
-- 4. CREAR TABLA PARA ASIGNACIÃ“N DE CONTADOR GENERAL POR EMPRESA (Configurable)
-- ============================================================================
...
-- 6. ACTUALIZAR RLS POLICIES PARA VIÃTICOS
-- ============================================================================
ALTER TABLE viaticos ENABLE ROW LEVEL SECURITY;

-- Policy: Persona ve solo sus viÃ¡ticos
CREATE POLICY "Persona ve solo sus viÃ¡ticos" ON viaticos
FOR SELECT USING (
  person_id = auth.uid()
...
-- Policy: Insertar viÃ¡ticos (solo persona autorizada o admin)
CREATE POLICY "Crear viÃ¡tico" ON viaticos
...
-- Policy: Actualizar viÃ¡tico (contador_general puede aprobar)
CREATE POLICY "Actualizar viÃ¡tico" ON viaticos
FOR UPDATE USING (
  EXISTS (
    SELECT 1 FROM company_members
    WHERE company_id = viaticos.company_id
...
-- Policy: Contador puede ver su asignaciÃ³n
CREATE POLICY "Contador ve su asignaciÃ³n" ON contador_general_assignments
...
-- ============================================================================
COMMENT ON TABLE viaticos IS 'ViÃ¡ticos de empleados: hospedaje, comidas, transporte, etc. Multi-persona por empresa.';
COMMENT ON TABLE contador_general_assignments IS 'AsignaciÃ³n configurable de contador general por empresa. Un contador puede tener mÃºltiples empresas.';
COMMENT ON COLUMN expenses.created_by IS 'Rastreo: quiÃ©n capturÃ³ el gasto (comprador). Nuevo en v1.0.';
COMMENT ON COLUMN viaticos.person_id IS 'QuiÃ©n solicita el viÃ¡tico (puede diferir de created_by si supervisor lo crea).';
COMMENT ON COLUMN viaticos.approval_reason IS 'Si rechazado: motivo (monto excesivo, categorÃ­a incorrecta, etc.).';
...
req.on('error', (err) => {
  console.log('ℹ️ Migración no se ejecutó automáticamente (esto es normal)');
  console.log('   La BD se actualizará cuando accedas a Supabase Studio');
});
